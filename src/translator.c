...
static int _generate_text_section(tree_t* node, FILE* output) {
    ...
    if (node->token->t_type == START_TOKEN) {
        ...
        for (tree_t* child = scope->first_child; child; child = child->next_sibling) {
            switch(child->token->t_type) {
                case INT_TYPE_TOKEN:
                    if (child->first_child->next_sibling->token->t_type != INT_VALUE_TOKEN) {
                        _generate_expression(child->first_child->next_sibling, output);
                        fprintf(output, "mov [%s], eax\n", child->first_child->token->value);
                    }
                    break;
                case WHILE_TOKEN: _generate_while(child, output); break;
                case IF_TOKEN: _generate_if(child, output); break;
                case SYSCALL_TOKEN: _generate_syscall(child, output); break;
                case ASIGN_TOKEN: _generate_assignment(child, output); break;
                case EXIT_TOKEN:
