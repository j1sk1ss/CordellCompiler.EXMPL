...
static char_type_t get_char_type(unsigned char ch) {
...
static token_t* create_token(token_type_t type, const unsigned char* value, size_t len) {
...
static int add_token(token_t** head, token_t** tail, token_type_t type, const unsigned char* buffer, size_t len) {
    token_t* new_token = create_token(type, buffer, len);
...
token_t* tknz_tokenize(int fd) {
    ...
    while ((bytes_read = pread(fd, buffer, BUFFER_SIZE, file_offset)) > 0) {
        ...
        for (ssize_t i = 0; i < bytes_read; ++i) {
            ...
            char_type_t ct = get_char_type(ch);
            ...
            if (ct != CHAR_SPACE && ct != CHAR_NEWLINE) {
                ...
                if (in_token) {
                    if (current_type != new_type && current_type != UNKNOWN_STRING_TOKEN) {
                        if (!add_token(&head, &tail, current_type, token_buf, token_len)) goto error;
                        in_token = 0;
                    }
                }
